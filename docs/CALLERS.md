...
Contract only — no spool/Bluetooth internals. Deeper route notes for maintainers: [API.md](API.md).
...
|--------|----------|--------|
| HA package / same host as the add-on | `http://127.0.0.1:8080` | What `ha/cat_printer.yaml` uses |
| Other self-hosted services on the LAN | `http://home.lan:8080` | Fine for **internal** traffic only |
| Anything that might be externally exposed, phone WAN, or standalone clients | `https://print.wunderfrog.com` | Prefer this hostname; do not hard-code `home.lan` into public or portable callers |

`home.lan:8080` is LAN convenience. If the caller could leave the house network (or you want one stable name for apps/scripts you share), use **`print.wunderfrog.com`**.
...
Examples below use `https://print.wunderfrog.com` as the portable default. Swap to `http://home.lan:8080` for LAN-only services, or `http://127.0.0.1:8080` from HA on the same host.

### PowerShell

```powershell
$base = "https://print.wunderfrog.com"
# $base = "http://home.lan:8080"   # LAN-only self-hosted callers
...
curl -s -X POST "$BASE/print/reddit" \
  -H "Content-Type: application/json" \
  -d '{"subreddit":"chonkers"}'

curl -s -X POST "$BASE/print/markdown" \
  -H "Content-Type: application/json" \
  -d '{"markdown":"# List\n\n- milk\n- eggs"}'

curl -s -X POST "$BASE/print/image" \
  -F "file=@./photo.jpg"
...
HA on the same host should keep using loopback:
